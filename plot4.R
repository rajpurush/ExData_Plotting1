...
# Converts date time string to POSIX type for plots
datetimestr <- paste(df$Date, df$Time)
datetime <- strptime(datetimestr, "%d/%m/%Y %H:%M:%S")

# Set plot of 2 rows and 2 columns
par(mfrow=c(2,2))

# First plot
plot(datetime, df$Global_active_power, ylab="Global Active Power", xlab="", type="n")
lines(datetime, df$Global_active_power)  

# second plot
plot(datetime, df$Voltage, ylab="Voltage", xlab="datetime", type="n")
lines(datetime, df$Voltage)  

# Third plot
plot(datetime, df$Sub_metering_1, ylab="Energy sub Metering", xlab="", type="n")
...
# fourth plot
plot(datetime, df$Global_reactive_power, ylab="Global_reactive_power", xlab="datetime", type="n")
